$(document).ready(function () {
    ...
    if ($.cookie('color')) {
        $('#color').attr('href', '../static/css/colors/' + $.cookie('color'));
    }
    $(document).on('click', '.demo .color',function (e) {
        e.preventDefault();
        $.cookie('color', $(this).data('color'), { expires: 7, path: '/' });
        $('#color').attr('href', '../static/css/colors/' + $(this).data('color'));
    });
    ...
    $(document).on('click', '.demo .boxed', function (e) {
        ...
        window.location.href = '_HTML_BODY.html';
    });

    if ($.cookie('background')) {
        if ($.cookie('box')) {
            $('body').css({ 'background-image': 'url(../static/Theme/img/bg/' + $.cookie('background') + ')' });
        }
    }
    $(document).on('click', '.demo .background', function (e) {
        ...
        if ($.cookie('box')) {
            ...
            $('body').css({ 'background-image': 'url(../static/Theme/img/bg/' + $(this).data('background') + ')' });
        }
    });
    ...
    var html = '' +
        ...
        '</a>' +
        '<h5>Выбери оформление</h5>' +
        // {{ url_for("static", filename="css/colors/red.css") }}
        ...
        '<a href="#" class="color" data-color="indigo.css" style="background-color:#303f9f"></a>' +
        '<a href="#" class="color" data-color="violet.css" style="background-color:#7b1fa2"></a>' +
        ...
        // '<hr />' +
        // '<a href="#" class="reset btn btn-sm btn-info p-l-30 p-r-30">Стандартное</a>' +
        '</div>';
    $('body').append(html);
});